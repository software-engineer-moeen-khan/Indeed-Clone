@extends('v2.auth.app')
@section('title') Create Account - Best Way Jobs @endsection
...
<div class="bg-white rounded-xl shadow-sm p-7 sm:p-9 max-w-md w-full space-y-6 border border-[#d4d2d0]">
    <div class="space-y-2">
        ...
        <p class="text-sm text-[#595959]">Build your profile and start applying for opportunities on Best Way Jobs.</p>
    </div>
    ...
    <form action="{{ route('register') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" id="name" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" placeholder="Your full name" class="indeed-input">
        </div>

        <div>
            <label for="email" class="block text-sm font-semibold text-[#2d2d2d] mb-2">Email address</label>
            <input type="email" id="email" name="email" value="{{ old('email') }}" required autocomplete="email" placeholder="name@example.com" class="indeed-input">
        </div>

        <div>
            <label for="password" class="block text-sm font-semibold text-[#2d2d2d] mb-2">Password</label>
            <input type="password" id="password" name="password" required autocomplete="new-password" placeholder="Create a strong password" class="indeed-input">
        ...
        <div>
            ...
            <input type="password" id="password_confirmation" name="password_confirmation" required autocomplete="new-password" placeholder="Repeat your password" class="indeed-input">
        </div>
